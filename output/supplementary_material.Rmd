---
title: "**Appendix**"
subtitle: '**Effect of Tocilizumab, Sarilumab, and Baricitinib on Mortality Among Patients Hospitalized for COVID-19 Treated with Corticosteroids: A Systematic Review and Meta-Analysis**'
output:
  pdf_document: 
    keep_tex: true 
fontsize: 12pt
...
mr_bari_optimistic_toci_direct_direct_sens = readRDS(
  here::here("output", "fits", "sensitivity_analyses",
...
# Sensitivity analysis (including all patients in RECOVERY Bari)

mr_bari_skeptical_sens = readRDS(here::here("output", "fits",
                                       "mr_bari_skeptical_sens.Rds"))
mr_bari_optimistic_bari_direct_sens = readRDS(here::here("output", "fits",
                                             "mr_bari_optimistic_bari_direct_sens.Rds"))
mr_bari_optimistic_toci_direct_sens = readRDS(here::here("output", "fits",
                                             "mr_bari_optimistic_toci_direct_sens.Rds"))
mr_bari_vague_sens = readRDS(here::here("output", "fits",
                                   "mr_bari_vague_sens.Rds"))
...
comparisons.[@zotero-3144;@karampitsakos2022] Thus, we decided to estimate
noninferiority probabilities of sarilumab and baricitinib vs. tocilizumab mostly
through indirect (across-trials) comparisons. Because of the fragility of such
...
available.[@dias2018] However, this approach does not allow one to assign prior
distributions to specific comparisons within the network, such as an indirect
effect. Further, the primary reason for not performing a Bayesian NMA is the
lack of direct evidence between several of the arms forcing a reliance on the
indirect evidence with no means of assessing its compatible with direct evidence.
We thus sought to perform multiple sensitivity analyses with different priors
to account for differing beliefs of non-inferiority. To overcome these
limitations, we fitted Bayesian random-effect meta-regression models to estimate
the indirect differences mentioned above (two separate sets of models: tocilizumab
vs. baricitinib; tocilizumab vs. sarilumab).[@pitchforth2012] These models are
identical to meta-regressions with a dichotomous covariate used to evaluate
subgroup differences.[@thompson2002] This approach allows one to apply a
plethora of prior distributions specifically to the indirect comparison
between treatments. 
...
We had originally planned to only apply $4$ prior
distributions for $\beta_{1, T}$ in each model (sarilumab and baricitinib's
models). However, as further explained later in the 
**"Post-hoc Analyses"** Section, we decided to incorporate new direct evidence
between baricitinib/sarilumab vs. tocilizumab into our analyses. We will now 
describe the $4$ originally prespecified priors. However, we note that, from
these, only results from *"Skeptical"* and *"Vague"* models are presented in the
main meta-regression figure/table in this manuscript. Other are still shown, but
as supplementary material.

In the model comparing **sarilumab** to tocilizumab, the originally prespecified
beliefs are: 
...
Results from this prior take part in the main meta-regression figure in
this article. 
...
Results from this prior take part in the Appendix meta-regression figure in
this article. 
...
Prior distributions for $\beta_{1, Sari}$ (ratio of odds ratios) in models
...
In the model comparing **baricitinib** to tocilizumab, the originally 
...
Prior distributions for $\beta_{1, Bari}$ (ratio of odds ratios) in models
...
In summary, both originally prespecified sarilumab and baricitinib models can
be fully described as:
...
One could argue that baricitinib's priors are stronger than sarilumab's given
differences on the mean values ($|0.105| > |0.049|$). We note that, although 
the mean values are different because we only originally found prior evidence 
for the sarilumab vs. tocilizumab comparison, these prior discrepancies are
likely to be of little relevance. As depicted in the figures above, the
baricitinib's priors have a greater mean but lower density than sarilumab's
priors. Therefore, we do not expect these priors' impact on each marginal
posterior distribution to be relevantly different.

\newpage

### Post-hoc Analyses

Regarding the **baricitinib vs. tocilizumab** meta-regression model, we also performed
a post-hoc analysis with a different set of priors. 

We fitted the exact same model, but with different “Optimistic for Baricitinib"
and “Optimistic for Tocilizumab" priors (hereafter, “Optimistic for Baricitinib
[Karampitsakos et al.]" and “Optimistic for Tocilizumab [inverse 
Karampitsakos et al.])":
...
These priors were constructed to reflect the exact same strength of the single
study that directly compared baricitinib to tocilizumab.[@karampitsakos2022] All
patients were treated with corticosteroids. The authors provided the following
28-day mortality data by email request:

- 50 patients in the tocilizumab arm died (out of 126 patients); 40 patients in
the baricitinib arm died (out of 125). 
...
We thus estimated a crude odds ratio of mean $=$ `r round(mean_log_bari,3)` and
standard error $=$ `r round(bari_sd_optimistic_bari, 3)` with the R package
*metafor*. **Results from these priors take part in the main meta-regression figure in this article.**
...
We fitted the exact same model, but with different “Optimistic for Sarilumab"
and “Optimistic for Tocilizumab" priors (hereafter, “Optimistic for Sarilumab
[REMAP-CAP]" and “Optimistic for Tocilizumab [inverse REMAP-CAP])":
...
**Results from these priors take part in the main meta-regression figure in this article.**
...
## Appendix Table 1: Trial Characteristics (Part 1)

```{r}
pre_table1 |>  
  # Re-order columns
  dplyr::select(study_name, intervention, 
                article_type, amount_sites, mortality_followup, dose) |> 
...
## Appendix Table 1: Trial Characteristics (Part 2)
...
## Appendix Figure 1: Risk of Bias Assessment
...
## Appendix Figure 2: Risk of Bias Proportion
...
## Appendix Figure 3: Meta-Analyses: Posterior Probabilities
...
## Appendix Figure 4: Baricitinib Sensitivity Meta-Analysis
...
## Appendix Figure 5: Posterior Between-Study Standard Deviations
...
## Appendix Table 2:  Posterior Probabilities of Heterogeneity
...
## Appendix Table 3: Meta-Analyses: Summary Results
...
## Appendix Figure 6: Funnel Plots
...
## Appendix Table 4: Certainty of Evidence (GRADE) 
...
grade |> 
  ...
  kableExtra::kable_styling(latex_options = "striped",
                            ...
                            full_width = T,
                            font_size = 10) |> 
  kableExtra::column_spec(1, width = "0.5cm")
```
...
## Appendix Figure 7: Baricitinib vs. Tocilizumab Meta-Regression Sensitivity Analysis
...
```{r meta regressions sensitivity bari plot, eval=FALSE, include=FALSE}
draws_bari =
  dplyr::tibble(
...
# Originally prespecified priors: 
...
p1 = plot_fun(parameter = parameter_fun(mr_bari_optimistic_bari),
...
p2 = plot_fun(parameter = parameter_fun(mr_bari_optimistic_toci),
              color = pal_toci[6],
              prior_mean = -mean_log_bari,
...
p1 / p2 + 
  patchwork::inset_element(arrows_bari,
                           ignore_tag = TRUE,
                           align_to = "full",
                           left = unit(2.5, 'cm'),
                           bottom = unit(0.2, 'cm'),
...
```{r meta regressions sensitivity plot bari display}
cowplot::ggdraw() + 
  # https://stackoverflow.com/questions/50026862/the-draw-image-function-from-cowplot-results-in-blurred-pdfs
  cowplot::draw_image(
    magick::image_read_pdf(
...
Post-hoc sensitivity analysis on baricitinib vs. tocilizumab meta-regression model
(ratio of odds ratios) with the originally prespecificied (less informative) priors.
...
percentages on top of each panel. Interval bars depict the posterior median and
95% credible intervals. Solid gray lines represent underlying prior distributions.
Each belief is labeled on top of each figure.

Underlying prior distributions: “Optimistic for Baricitinib",
$\operatorname{Normal}(-0.105, 0.416^2)$; “Optimistic for Tocilizumab",
$\operatorname{Normal}(0.105, 0.416^2)$


\newpage

## Appendix Table 5: Baricitinib vs. Tocilizumab Meta-Regression Sensitivity Analysis

```{r meta regression sensitivity analysis bari table}
...
results_bari = dplyr::left_join(ror_bari, proba_bari) |> 
  dplyr::mutate(
    ...
    Belief = factor(Belief, levels = c("Optimistic for Baricitinib",
...
names(results_bari)[3] = 
  paste0(names(results_bari)[3],
         " (1)")

names(results_bari)[4] = 
  paste0(names(results_bari)[4],
         " (2)")

results_bari |> 
  # https://stackoverflow.com/questions/53794142/increase-line-row-spacing-with-kableextra
  kableExtra::kbl(booktabs = T, linesep = "\\addlinespace") |>
  kableExtra::kable_styling(latex_options = "hold_position",
                            full_width = T) |> 
  kableExtra::footnote(number = c("Posterior probability below the noninferiority margin (1.14 ROR)",
                                  "Posterior probability below 1.0 ROR"),
                       general = "Abbreviations: ROR, ratio of odds ratios; CrI, credible interval")
...
## Appendix Figure 8: Sarilumab vs. Tocilizumab Meta-Regression Sensitivity Analysis
...
p1 = plot_fun(parameter = parameter_fun(mr_sari_optimistic_sari),
...
p2 = plot_fun(parameter = parameter_fun(mr_sari_optimistic_toci),
              color = pal_toci[6],
              prior_mean = -mean_log_sari,
...
```{r meta regressions sensitivity plot sari display}
cowplot::ggdraw() + 
  # https://stackoverflow.com/questions/50026862/the-draw-image-function-from-cowplot-results-in-blurred-pdfs
  cowplot::draw_image(
    magick::image_read_pdf(
...
$\operatorname{Normal}(-0.049, 0.193^2)$; “Optimistic for Tocilizumab",
$\operatorname{Normal}(0.049, 0.193^2)$


\newpage

## Appendix Table 6: Sarilumab vs. Tocilizumab Meta-Regression Sensitivity Analysis
...
results_sari = dplyr::left_join(ror_sari, proba_sari) |> 
  dplyr::mutate(
    Belief = dplyr::case_when(
      ...
      Belief == "optimistic_toci" ~ "Optimistic for Tocilizumab"),
    Belief = factor(Belief, levels = c("Optimistic for Sarilumab",
                                       "Optimistic for Tocilizumab"))
    ) |> 
  dplyr::arrange(Belief)
...
## Appendix Table 7: Baricitinib vs. Tocilizumab Sensitivity Analysis (RECOVERY Bari)
...
draws_bari_sens =
  dplyr::tibble(
    ...
    "optimistic_bari" = parameter_fun(mr_bari_optimistic_bari_direct_sens)$value,
    "optimistic_toci" = parameter_fun(mr_bari_optimistic_toci_direct_sens)$value
  ) |> 
...
Sensitivity analysis including all patients treated with corticosteroids,
regardless of tocilizumab use, in RECOVERY Bari. 

Underlying prior distributions: 
"Skeptical",
$\operatorname{Normal}(0, 0.354^2)$;
“Optimistic for Baricitinib (Karampitsakos et al.)",
$\operatorname{Normal}(-0.335, 0.264^2)$;
“Optimistic for Tocilizumab (inverse Karampitsakos et al.)",
$\operatorname{Normal}(0.335, 0.264^2)$;
"Vague", 
