...
author: "Arthur M. Albuquerque, Igor Eckert BS, Lucas Tramujas MD, Guillaume Butler-Laporte MD, Emily G. McDonald MD MSc, James M. Brophy MD PhD, Todd C. Lee MD MPH"
...
Then, the posterior predictive distribution can be described as:

\begin{align*}
\theta_{new} & \sim Normal(\mu, \tau^2)\\
\end{align*}

where $\theta_{new}$ "relates to the effect in a new study that is deemed
‘sufficiently similar’ to the existing studies to be eligible for the analysis".

Because these are Bayesian models, we need to specify prior distributions for all parameters. We applied a prior for $\mu$ that cover plausible values, assigning limited density to unlikely values, and thereby exerting little influence on the results in all models.  A simulation study supported by empirical evidence suggests that a prior with a 95\% interval between $0.23$ and $4.35$ is suitable to this case.\cite{pedroza2018a} In the log scale, such prior is described as:
...
Regarding the between-study standard deviation parameter ($\tau$), we used an informative prior based on previous evidence. We applied the predictive distribution on all-cause mortality for “placebo / control" vs. “pharmacological" comparisons derived by Turner et al.\cite{turner2015a}. Although the
definition of small or large between-study heterogeneity is
arbitrary, previous work suggests cutoff values (“reasonable” heterogeneity
between 0.1 and 0.5, and “fairly high” between 0.5 and 1.0. We added a category for low heterogeneity (between 0 and 0.1):

\begin{align*}
\tau & \sim \operatorname{Log-Normal}(-1.975, 0.67^2)
\end{align*}

```{r informative tau prior visualization, fig.height=3.2, fig.width=5, fig.cap="This distribution concentrates 31% of probability density between 0 and 0.1 log odds ratio ('low heterogeneity'), 66% between 0.1 and 0.5  ('reasonable'), and only 4% between 0.5 and 1.0 ('fairly high')."}
...
using a less informative prior for $\tau$:

\begin{align*}
\tau & \sim \operatorname{Half-Normal}(0, 0.5^2)
\end{align*}

This distribution yields higher heterogeneity probabilities, while limiting less plausible high levels.

```{r weakly tau prior visualization, fig.height=3, fig.width=5, fig.cap="This distribution concentrates 16% of probability density between 0 and 0.1 log odds ratio ('low heterogeneity'), 52% between 0.1 and 0.5  ('reasonable'), and 27% between 0.5 and 1.0 ('fairly high')."}
...
that is exchangeable with the studies included in our meta-analysis”.
...
calculations or prior distribution elicitation in future RCTs.
This generates new trial population parameter estimates, independently of
sample size and other population characteristics.


### Meta-regression

Our primary research question is whether baricitinib and sarilumab are noninferior to tocilizumab. A sensitive approach to answer this question is to separately meta-analyze direct comparisons between tocilizumab vs. baricitinib and tocilizumab vs. sarilumab. However, a preliminary search by our research group showed a limited number of studies that report such comparisons. Thus, we decided to only estimate the noninferiority probability of sarilumab and baricitinib vs. tocilizumab through indirect (across-trials) comparisons. Because of the fragility of such comparisons, it is of great interest to test the robustness of these estimates with multiple prior distributions.

Bayesian network meta-analyses are historically used to estimate direct, indirect, and mixed-effects when more than two treatment comparisons are available. However, this approach does not allow one to assign prior distributions to specific comparisons within the network, such as an indirect effect. To overcome this limitation, we fitted Bayesian random-effect meta-regression models to estimate the indirect differences mentioned above (two separate sets of models: tocilizumab vs. baricitinib; tocilizumab vs. sarilumab). These models are identical to meta-regressions with a dichotomous covariate used to evaluate subgroup differences. This approach allows one to apply a plethora of prior distributions specifically to the indirect comparison between treatments. 

Both baricitinib and sarilumab models are identical, and are described as:
...
the same informative prior mentioned before:

\begin{align*}
\tau & \sim \operatorname{Log-Normal}(-1.975, 0.67^2)
\end{align*}

In addition to the parameters mentioned above, we should also assign a prior distribution for the indirect comparison parameter ($\beta_{1, T}$). Inspired by the guidelines provided by Zampieri et al.,\cite{zampieri2021} we fitted different
...
```{r all priors sari meta regressions, fig.height=6, fig.width=5, fig.cap="Prior distributions for $beta_{1, Sari}$ (ratio of odds ratios) in models comparing sarilumab to tocilizumab. Panel A: 'Skeptical' prior. The Normal($0$, $0.354^2$) distribution approximately concentrates $95$% of probability density between $0.5$ and $2.0$ in the linear scale ($-0.69$ and $0.69$ in the log scale, respectively). Panel B: 'Optimistic for Sarilumab' prior.  The Normal($-0.049$, $0.193^2$) distribution approximately concentrates $40$% of probability density above $1$. Panel C: 'Optimistic for Tocilizumab' prior.  The Normal($0.049$, $0.193^2$) distribution approximately concentrates $40$% of probability density below $1$."}
...
* “Skeptical", which expects no difference between baricitinib and tocilizumab studies (mean centered at $1$) with $95\%$ probability between $0.5$ and $2$ (linear scale, ratio of odds ratios). These values represent a weak belief strength,\cite{zampieri2021} based on the little information currently available on this topic. $\operatorname{Normal}(0, 0.354^2)$

* “Optimistic for Baricitinib", which expects a greater mortality reduction due to baricitinib use in comparison to tocilizumab. The mean is centered at $0.9$ (linear scale, ratio of odds ratios), which was arbitrarily chosen based on clinical judgment that a 10\% relative difference is clinically important. Because there was no information previously available, we considered that there is 40\% of probability that the ratio of odds ratios is greater than 1, representing a weak belief strength.\cite{zampieri2021} $\operatorname{Normal}(-0.105, 0.416^2)$

* “Optimistic for Tocilizumab", which expects a greater mortality reduction due to tocilizumab use in comparison to sarilumab. The mean is centered at $1.11$, the reciprocal of $0.9$. Similarly, this prior belief will also have a weak strength, represented by 40\% of probability that the ratio of odds ratios is lower than $1$.\cite{zampieri2021} $\operatorname{Normal}(0.105, 0.416^2)$
...
```{r fig.height=6, fig.width=5, fig.cap = "Prior distributions for $beta_{1, Bari}$ (ratio of odds ratios) in models comparing baricitinib to tocilizumab. Panel A: 'Skeptical' prior. The Normal($0$, $0.354^2$) distribution approximately concentrates $95$% of probability density between $0.5$ and $2.0$ in the linear scale ($-0.69$ and $0.69$ in the log scale, respectively). Panel B: 'Optimistic for Baricitinib' prior.  The Normal($-0.105$, $0.41^2$) distribution approximately concentrates $40$% of probability density above $1$. Panel C: 'Optimistic for Tocilizumab' prior.  The Normal($0.105$, $0.41^2$) distribution approximately concentrates $40$% of probability density below $1$."}
...
These priors were constructed to reflect the exact same strength of the single study that directly compared sarilumab to tocilizumab.\cite{remap_preprint}

The new standard deviation of $0.118$ was obtained through the formula:\cite{cochrane632}
...
One major criteria in noninferiority analyses is that superiority of active comparator  --- in this case, tocilizumab --- in comparison to control has been shown.\cite{tsui2019}. The WHO meta-analysis presented results in favor of tocilizumab's superiority.\cite{who_jama} Thus, we will use the following formula to estimate our noninferiority margin:\cite{trone2020}

\begin{align*}
\gamma =  \left( \frac{1}{\theta} \right)^{1 - x} \\
\end{align*}

where $\gamma$ is the estimated noninferiority margin, $\theta$ is the tocilizumab's mean effect size, and $x$ is the percentage of tocilizumab's effect that is desired to be preserved. Regarding $\theta$, we extracted the mean odds ratio regarding tocilizumab's effect in the subgroup of patients using corticosteroids (page 14 in their Supplement 2 \cite{who_jama}). We chose this subgroup because it represents our population of interest. Moreover, we will follow the U.S. Department of Health and Human Services Food and Drug Administration's recommendation and define $x$ as $50\%$ for our primary noninferiority analysis.\cite{tsui2019, fda} Thus, our main noninferiority margin (in the linear scale) will be:
...
PRISMA 2020 flow diagram with results from our search process. 
...
toci_mu =
  ma_toci |> 
  tidybayes::tidy_draws() |> 
  dplyr::summarise("Tocilizumab" = exp(b_Intercept))

bari_mu =
  ma_bari |> 
  tidybayes::tidy_draws() |> 
  dplyr::summarise("Baricitinib" = exp(b_Intercept))

sari_mu =
  ma_sari |> 
  tidybayes::tidy_draws() |> 
  dplyr::summarise("Sarilumab" = exp(b_Intercept))
...
```{r posterior probabilities heatmap, fig.width=9}
...
mu_pred_probabilities = 
  ...
  dplyr::summarise("Pr(< 1.0)" = mean(value < 1),
                   "Pr(< 0.9)" = mean(value < 0.9)) |> 
  dplyr::ungroup() |> 
...
probs_plot = function(effect, ylab = F, ttitle){
  ...
  {if (ylab) labs(x = "\nPosterior Probability Cutpoints")} + 
  facet_wrap(~prior) +
...
plot_RD = function(data, color1, color2, ylab = F, Title){
  data |>  
  dplyr::select(-OR) |>   
  tidyr::pivot_longer(
    cols = everything(),
...
p1 = plot_RD(toci_RD, pal_toci[4], pal_toci[6], ylab = T, Title =  "\nTocilizumab")
p2 = plot_RD(bari_RD, pal_bari_sari[4], pal_bari_sari[3], Title = "\nBaricitinib")
p3 = plot_RD(sari_RD, pal_bari_sari[6], pal_bari_sari[7], Title = "\nSarilumab")
...
Meta-analysis average effect posterior distributions in the risk difference scale (Y-axis) across multiple mortality risks under control treatment (X-axis) for each experimental drug. Black solid lines represent the median, darker colored areas represent the 80% CrIs, and lighter areas represent the 95% CrIs. Risk difference greater than 0 favors experimental drug over control.
...
Posterior probabilities of benefit or equivalence in the risk difference scale.
Each line represents the posterior probability for a specific cutpoint, such as 
...
Between study standard deviation ($\tau$) posterior distributions  upon
...
#### Posterior Probabilities of Heterogeneity in Each Meta-Analysis
...
#### Posterior Credible Intervals in Each Meta-Analysis 
...
par(mfrow = c(1, 3), # 3 figures, side by side ("1 row, 3 columns")
    # Margins
    mar=c(6, # bottom
          4, # left
          3, # top
...
Color filled curves represent the posterior distributions. Color filled areas
...
#### Posterior Credible Intervals, Sarilumab Meta-Regression Sensitivity Analysis
...
d_bari_plot =
  ...
  geom_segment(aes(x = 0, y = 0.9, xend = log(1.16), yend = 0.15),
               color = "gray70",
               linetype = 3,
               size = 0.5) +
  geom_segment(aes(x = log(1.28), y = 1, xend = log(1.8), yend = 0.815),
               color = "gray70",
               linetype = 3,
               size = 0.5) +
...
p2 = 
  ...
  scale_x_continuous(breaks = log(seq(1, 1.28, 0.07)),
                     labels = seq(1, 1.28, 0.07)) +
  scale_y_continuous(
      breaks = seq(from = 0.9, to = 1, 0.025),
      labels = c("90", "92.5", "95", "97.5", "100"),
      expand = c(0, .03)
    ) +
  coord_cartesian(y = c(0.925, 0.98),
                  x = log(c(1, 1.3))) +
  ...
  theme(legend.position = "none",
      ...
      panel.background = element_rect(colour = "gray40", size=1.5))
```
...
(
  d_bari_plot + 
  inset_element(p2, 0.25, 0.15, 0.95, 0.8, ignore_tag = TRUE) 
  ) +
